...
export default function CouponForm({ coupon, mode }: CouponFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {      setIsSubmitting(true)
      ...
      if (mode === 'create') {
        response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/coupons`, {
          method: 'POST',
          headers: {
        ...
        response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/coupons/${couponData.id_kupon}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
