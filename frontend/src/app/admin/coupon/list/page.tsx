...
export default function AdminCouponListPage() {
  ...
  useEffect(() => {
    const checkAuthAndFetch = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/coupons`, {
          method: 'GET',
          headers: {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this coupon?')) {
      try {
        const token = sessionStorage.getItem('jwtToken')
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/coupons/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
          }
